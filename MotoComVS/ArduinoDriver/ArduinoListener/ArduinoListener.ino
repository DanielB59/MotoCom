...
const byte CMD_USERCMD                = 0x17;
const byte ACK_USERCMD                = 0x18;
...
unsigned int userCommand;
unsigned int userCommandResult;
...
unsigned int executeUserCommand(byte command) {
	if (0x70 == command)
		digitalWrite(2, HIGH);
...
void loop() {
  ...
  switch (commandByte) {
    ...
    case CMD_SHIFTIN:
      ...
      break;
	case CMD_USERCMD:
	  userCommandResult = executeUserCommand(data[2]);
	  Serial.write(START_OF_RESPONSE_MARKER);
	  Serial.write(3);
	  Serial.write(ACK_USERCMD);
	  Serial.write(data[2]);
      Serial.write(userCommandResult);
      Serial.flush();
	  switch (data[2]) {
		case 0x00:
